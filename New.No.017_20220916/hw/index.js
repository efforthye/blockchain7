...
startBtn.onclick = () =>{
    ...
    // 버튼을 한번 더 눌렀을 때 코인을 추가로 차감하지 않고 리턴해준다.
    // startBtn.onclick=()=>{
    //     return;
    // }
    ...
    console.log(startBtn.classList); // .start ok
    ...
    if(startBtn.getElementsByClassName("start")){
        ...
        setTimeout(() => {
            ...
            setTimeout(()=>{
                ...
                console.log(`circleImgElem.classList : ${circleImgElem.classList}`);
            }, 600);
    ...
    // 0~2중에 랜덤으로 배열의 n번째를 정해 컴퓨터RPS에 넣어준다.



    // 게임이 끝나면 게임 진행 상태를 바꾸고 초기화 시켜줘야 한다.
    // game = 0;
    // 컴퓨터 RPS도 초기화 시켜준다.
    // return;
...
function rockPaperScissors(rps){
    ...
    // 가위바위보를 여러번 누르지 못하게 해야한다. (어떻게 해야할지 아직 모름 )
    // scissorsElem.onclick = () =>{
    //     return
    // }
    // rockElem.onclick = () =>{
    ...
    if(comRPS=="s"){
        console.log("컴퓨터 가위");
        circleImgElem.style.backgroundImage = `url("../KakaoTalk_20220918_124352892_01.jpg")`;
    }else if(comRPS=="r"){
        console.log("컴퓨터 바위");
        circleImgElem.style.backgroundImage = `url("../KakaoTalk_20220918_124352892.jpg")`;
    }else{
        console.log("컴퓨터 보");
        circleImgElem.style.backgroundImage = `url("../KakaoTalk_20220918_124352892_02.jpg")`;
    }
    ...
    if(comRPS==userRPS){
        ...
        // rockPaperScissors(rps); //무한루프에 빠짐
        ...
        if(winLoose=="loose"){
            ...
            // 여기
            // 시간이 되면 1초에 걸쳐서 위의 코인이 돌아가고 코인이 스르륵 올라가고 
            // 만약 윈넘이 20이면 왕창 터지는 모션도 넣어주고.. 그렇게 해주기..
            ...
            setTimeout(()=>{
                ...
                roulet[0].style.rotate=`${(rouletteRanNum+1)*-22.5}deg`;
                // roulet[0].style.rotate=`${(rouletteRanNum+1)*(Math.floor(Math.random()*3)+1)*-22.5}deg`;
                console.log(roulet[0].style.rotate);

                // 부드럽게 돌려준다.
                roulet[0].style.transition="all 2s";
...
function init(){
    ...
    startBtn.classList.remove("start");
    startBtn.classList.add("startBtn");
    console.log(`startBtn.classList : ${startBtn.classList}`);
